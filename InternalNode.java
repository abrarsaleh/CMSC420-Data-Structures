...
public class InternalNode extends Node {
	...
	public Node add(City city, Point2D.Float origin, int width, int height) {
		...
		for (int i = 0; i < 4; i++) {
			if (intersects(cityLocation, regions[i])) {
				children[i] = children[i].add(city, origins[i], halfWidth, halfHeight);
				break;
			}
		}
		return this;
	...
	@Override
	public Node remove(City city, Point2D.Float origin, int width, int height) {
		final Point2D cityLocation = city.toPoint2D();
		for (int i = 0; i < 4; i++) {
			if (intersects(cityLocation, regions[i])) {
				children[i] = children[i].remove(city, origins[i], halfWidth, halfHeight);
			}
		}
	...
	public static boolean intersects(Point2D point, Rectangle2D rect) {
		return (point.getX() >= rect.getMinX() && point.getX() < rect.getMaxX() && point.getY() >= rect.getMinY()
				&& point.getY() < rect.getMaxY());
	}
	...
	/**
	 * Gets the child node of this node according to which quadrant it falls in
	 * 
	 * @param quadrant
	 *            quadrant number (top left is 0, top right is 1, bottom left is
	 *            2, bottom right is 3)
	 * @return child node
	 */
	...
	/**
	 * Gets the rectangular region for the specified child node of this internal
	 * node.
	 * 
	 * @param quadrant
