...
public class MeeshQuest {
			...
			for (File file : files) {
				...
				if (file.isFile() && matcher.matches()) {
					...
					if (GENERATE_JUNIT_SRC) {
						...
						JUnitWriter.write("\tpublic void test_" + testSubName
								+ "() throws Exception {\n\t\trunXmlTest(\"" + testName + "\");\n\t}\n");
					}
				}
			}
	...
	public void processInput() {
		...
		try {
			...
			if (LOCAL_TEST) {
			 ...
			 doc = LOCAL_TEST ? XmlUtility.validateNoNamespace(xmlInput) : XmlUtility.validateNoNamespace(systemInput);

			//doc = XmlUtility.validateNoNamespace(new File("test2.xml"));
	...
	private void processCommand(final Element commandNode) throws IOException {
		...
		if (name.equals("commands")) {
			...
			if (LOCAL_TEST)
				System.out.println("Problem with the validator");
			System.exit(-1);
		}
	}
}
