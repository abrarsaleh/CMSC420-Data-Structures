...
/**
 * Node abstract class for a PR Quadtree. A node can either be an empty node, a
...
public abstract class Node {
	...
	/**
	 * Adds a city to the node. If an empty node, the node becomes a leaf node.
	 * If a leaf node already, the leaf node becomes an internal node and both
	 * cities are added to it. If an internal node, the city is added to the
	 * child whose quadrant the city is located within.
	 * 
	 * @param city
	 ...
	 * contained in it, the city is removed and the node becomes a leaf node. If
	 * this is an internal node, then the removal command is passed down to the
	 * child node whose quadrant the city falls in.
	 * 
	 * @param city
	 *            city to be removed
	 * @param origin
	 ...
	 */
	public abstract Node remove(City city, Point2D.Float origin, int width, int height);
